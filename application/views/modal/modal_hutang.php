...
if(!empty($_POST['act'])){ 
	...
	$data = database()->Query("SELECT * FROM db_jurnalumum WHERE rec_group='".$show['rec_group']."' and rec_module='21' and no_akun != '11' and no_akun != '21' ");
	$show = $data->fetch();
		
		$nomor_akun = database()->Query("SELECT * FROM db_noakun WHERE no_akun='".$show['no_akun']."' ");
		$nomor_akun = $nomor_akun->fetch();
	
		$cicilan = database()->Query("SELECT * FROM db_jurnalumum WHERE no_akun='11' and rec_module='21' and rec_group='".$show['rec_group']."' ")->Fetch();
...
<form method="POST" onSubmit="return modal_send(this)" action="data_proccess?pjax<?=url.$pg?>">
	<div id="data-saldo">
		...
		<input type="text" class="form-control" name="debet" value="<?=$debet?>">
			...
			<div class="col-md-6">
		<label>Saldo DP Hutang</label>
		<input type="text" class="form-control" name="dp" value="<?=$cicilan?>">
		</div>
		
		</div>
		
		<input name="proccess_type" value="hutang" style="display:none;"/>
		<?php
		if(!empty($stats)){ ?>
